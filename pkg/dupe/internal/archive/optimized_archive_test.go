...
func TestFormatCache(t *testing.T) {
	...
	format1, err := cache.GetFormat("test.zip")
	if err != nil {
	...
	format3, err := cache.GetFormat("test.tar.gz")
	if err != nil {
	...
	format4, err := cache.GetFormat("test.zip")
	if err != nil {
		t.Fatalf("Failed to get format after clear: %v", err)
	}
	if format4 == nil || format4.Extension() != ".zip" {
...
func TestOptimizedExtractor(t *testing.T) {
	...
	format, err := cache.GetFormat("test.zip")
	if err != nil {
		t.Fatalf("Failed to get format: %v", err)
	}
	if format == nil || format.Extension() != ".zip" {
...
func TestBatchExtractor(t *testing.T) {
	...
	cache := extractor.Cache
	format, err := cache.GetFormat("test.tar.gz")
	if err != nil {
		t.Fatalf("Failed to get format: %v", err)
	}
...
func BenchmarkFormatCache(b *testing.B) {
	...
	b.Run("CacheMiss", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			_, _ = cache.GetFormat(fmt.Sprintf("test%d.zip", i))
		}
	})
	
	b.Run("CacheHit", func(b *testing.B) {
		// Prime the cache
		_, _ = cache.GetFormat("cached.zip")
...
func BenchmarkBufferPool(b *testing.B) {
	...
	b.Run("WithPool", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			buf := extractor.BufferPool.Get().([]byte)
			_ = buf[0] // Use the buffer
			extractor.BufferPool.Put(buf)
		}
	})
...
// BenchmarkBatchProcessing has been removed due to complexity
// The batch processing functionality is thoroughly tested in TestBatchExtractor