...
func (fc *FormatCache) GetFormat(filename string) (archives.Format, error) {
	fc.Mu.RLock()
	...
	// Identify new format
	format, _, err := archives.Identify(fc.Context, filename, nil)
	if err != nil {
		return nil, err
	}
...
func (oe *OptimizedExtractor) ExtractWithCache(filename string, reader io.Reader, 
	handleFile func(fileInfo archives.FileInfo) error) error {
	
	// Get cached format
	format, err := oe.Cache.GetFormat(filename)
	if err != nil {
		return err
	}
	...
	return extractor.Extract(oe.Cache.Context, reader, func(ctx context.Context, fileInfo archives.FileInfo) error {
		if fileInfo.IsDir() {
			return nil
		}
		
		// Use buffer pool for reading
		buf := oe.BufferPool.Get().([]byte)
...
func (be *BatchExtractor) CacheArchiveFiles(filename string, reader io.Reader) error {
	format, err := be.Cache.GetFormat(filename)
	if err != nil {
		return err
	}
	...
	err = extractor.Extract(be.Cache.Context, reader, func(ctx context.Context, fileInfo archives.FileInfo) error {
		if !fileInfo.IsDir() {
...
func (be *BatchExtractor) ProcessCachedFiles(filename string, 
	processFunc func(fileInfo archives.FileInfo) error) error {
