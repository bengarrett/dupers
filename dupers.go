...
// Dupers is a blazing-fast duplicate file checker and filename matcher.
package dupers
...
// Bucket returns named bucket string as the Bucket type.
func Bucket(name string) parse.Bucket {
	return parse.Bucket(name)
}

// Print highlights the matched results of the database comparisons.
// Quiet reduces the feedback to stdout.
// Exact highlights only case-sensitive, matched terms.
// Term is the string within the matches to highlight.
func Print(quiet, exact bool, term string, m *database.Matches) string {
	return parse.Print(quiet, exact, term, m)
}

// PrintWalk prints to stdout the "Scanning/Looking up" file or item task statement.
...
// Config are the options for stdout verboseness.
type Config struct {
...
// CheckPaths counts both the number of files in a directory to check and the number of buckets.
...
// The checksum is a unique, SHA256 hash value generated by the file content.
...
// Read the named file archive (such as a compressed zip), walk, hash and save its content to the bucket.
// Supported archives: https://pkg.go.dev/github.com/mholt/archiver/v3#pkg-constants
...
// Read7Zip opens the named 7-Zip file archive, hashes and saves the content to the bucket.
...
// Remove duplicate files from the source directories.
...
// Removes the subdirectories and their content from the source which do not contain unique DOS or Windows programs.
...
// WalkDir walks the named bucket directory and looks for any new file checksums to save to the database.
...
// WalkDir walks the bucket directories and looks for any new file checksums to save to the database.
...
// WalkSource walks the source directory or a file for checksums and stores the finds in memory for future comparison.
...
// lookup the checksum value in config.compare and return the file path.
...
// skipFiles returns the value of config.sources as strings.
func (c *Config) skipFiles() (files []string) {
	files = append(files, c.Sources...)
	return files
}

// match returns the 'Found duplicate match' string.
...
// matchItem returns the 'Found duplicate match' string along with file stat info.
...
// printRM returns a "could not remove" string.
...
// removes directories that do not contain DOS or Windows programs.
...
// skipDir forces WalkDir to ignore specific system and hidden directories.
...
// skipFile returns true if the named file matches a known Windows or macOS system file.
...
// walkCompare walks the root directory and inserts the checksums of the files to config.compare.
...
// WalkArchiver walks the directory of the named bucket, saving the checksums of any new files to the database.
// Archived files supported by the archiver library will also have the content hashed.
// Archives contained within archives are not walked.
...
// findItem returns true if the absolute file path is in config.sources.
...
// listItems sets config.sources to list all the filenames used in the bucket.
